...
def _pitest_toolchain_impl(ctx):
    ...
    template_variables = platform_common.TemplateVariableInfo({
        "PITEST_BIN": target_tool_path,
    })
    default = DefaultInfo(
        files = depset(tool_files),
        runfiles = ctx.runfiles(files = tool_files),
    )
    pitestinfo = PiTestInfo(
...
pitest_toolchain = rule(
    implementation = _pitest_toolchain_impl,
    attrs = {
    ...
    doc = """Defines a pitest compiler/runtime toolchain.
